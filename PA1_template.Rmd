...
plot(rownames(intrvlSteps), intrvlSteps, type = "l", col = "darkolivegreen3", main = "Average dailyl activity pattern", xlab = "Interval", ylab = "Number of Steps")
```
...
```
The interval that, on average across all days in the dataset, contains the maximum number of steps is <b>interval `r maxInt`</b>.
...
```
The total number of missing values in the dataset is <b>`r misVal`</b>.
